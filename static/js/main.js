...
function startGame() {
  ...
  function loop() {
    const now = Date.now();
    const delta = (now - timePreviousLoop) / 1000.0;
    timePreviousLoop = now;
    const instFPS = delta > 0 ? 1 / delta : 0;
    fpsSmoothed = fpsSmoothed ? fpsSmoothed * 0.9 + instFPS * 0.1 : instFPS;
    if (fpsEl) fpsEl.textContent = "FPS: " + Math.round(fpsSmoothed);
    if (ctx) {
    ...
    if (me) {
      ...
      if (keys.w || keys.ArrowUp) {
        dx += Math.cos(me.angle) * moveSpeed;
        dy += Math.sin(me.angle) * moveSpeed;
      }
      if (keys.s || keys.ArrowDown) {
        dx -= Math.cos(me.angle) * (moveSpeed / 2);
        dy -= Math.sin(me.angle) * (moveSpeed / 2);
      }
      let moved = false;
      if (dx !== 0 || dy !== 0) {
        if (attemptMoveAndResolve(me, dx, dy)) moved = true;
      }
      if (keys.a || keys.ArrowLeft) {
        if (attemptRotate(me, -rotSpeed)) moved = true;
      }
    ...
    for (const [name, t] of Object.entries(STATE.tanks))
      drawTank(t.x, t.y, t.angle, t.color, name);
